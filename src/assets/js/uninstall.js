document.addEventListener('DOMContentLoaded', function () {
    ...
    modal.addEventListener('click', function (e) {
        if (e.target.type === 'radio') {
            ...
            document.querySelectorAll('.ozopanel-feedback-text, .ozopanel-feedback-alert').forEach(function (element) {
                element.style.display = 'none';
            });
            document.querySelector('.ozopanel-data-alert').style.display = 'block';
    ...
    modal.addEventListener('click', function (e) {
        if (e.target.tagName === 'BUTTON') {
            ...
            var input = document.querySelector('.ozopanel-feedback-text input[name="reason_' + reasonKey + '"]');
            var dataCollect = document.getElementById('ozopanel-data-collect').checked ? '1' : '';

            var nonce = document.querySelector('.ozopanel-form-style-one input[name="_wpnonce"]').value;
            ...
            if (reasonKey === 'none') {
                ...
                xhr.send('action=ozopanel_deactivate_feedback&submit=' + submit + '&data_collect=' + dataCollect + '&nonce=' + nonce + '&reason_key=' + reasonKey + '&reason=' + (input ? input.value.trim() : ''));
